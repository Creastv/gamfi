        ...
        <div class="row">
            <div class="col-md-12">
                <nav class="navbar navbar-expand-lg " itemscope itemtype="http://schema.org/SiteNavigationElement">
                    <?php if(get_field('logo-header', pll_current_language('slug'))): $logoHeader = get_field('logo-header', pll_current_language('slug')); ?>
                    ...
                    <?php if(get_field('pokaz_button', pll_current_language('slug'))) { $navPos = "justify-content-center"; } else{ $navPos = "justify-content-end"; } ?>

                    <?php get_template_part( 'templates/header/menu', get_post_format() ); ?>

                    <?php if(get_field('pokaz_button', pll_current_language('slug'))) { ?>

                    <?php if( have_rows('cta', pll_current_language('slug')) ): ?>
                    <div class="header-right">
                        <div class="inner">
                            <i class="fas fa-ellipsis-h opener"></i>
                        </div>
                        <?php while( have_rows('cta', pll_current_language('slug')) ): the_row(); ?>
                        <div class="wrap-cta">
                            <?php if(get_sub_field('link') && get_sub_field('tresc'))  { ?>
                            <a href="<?php the_sub_field('link', pll_current_language('slug')); ?>" class=" btn btn-md btn-revers"><?php the_sub_field('tresc', pll_current_language('slug')); ?></a>
                            <?php } ?>
                            <?php if(get_sub_field('link_two') && get_sub_field('tresc_two'))  { ?>
                            <a href="<?php the_sub_field('link_two', pll_current_language('slug')); ?>" class=" btn btn-md btn-main"><?php the_sub_field('tresc_two', pll_current_language('slug')); ?></a>
                            <?php } ?>
                        </div>
                        <?php endwhile; ?>
                    </div>
